...
namespace simol
{
  ...
  double SpaceSine::laplacian(DVec const& position) const
  {
    ...
    //return pow(pulsation_, 2) * ((amplitude_+4*coupling_) * cos(pulsation_ * position(0)) + (amplitude_+coupling_) * cos(pulsation_ * position(1)))
    //  - 5 * coupling_  * pow(pulsation_, 2) * cos(pulsation_ * position(0)) * cos(pulsation_*position(1));
  }

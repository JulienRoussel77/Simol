...
//#### Quadratic #####  
  ...
  LennardJones::LennardJones(Input const & input):
    ...
    B_spline_(untruncatedDerivative(splineRadius_)*(1-input.splineRatio())*input.cutOffRatio()*input.sigmaLJ())
  {}
  ...
  { 
    return -24 * epsLJ_/ sigmaLJ_ * (2*pow(sigmaLJ_/dist,13)-pow(sigmaLJ_/dist,7));
  }
